...
if __name__ == "__main__":
    ...
    a = g.node(pdf_extraction)
    g.node(filter_for_profanity, params={"words": ["fuck"]})
    g.node(object_detector)
    g.node(text_chunks, params={"chunk_size": 500})
    g.node(chunk_embeddings)

    g.edge(pdf_extraction, filter_for_profanity)
    g.edge(pdf_extraction, object_detector)

    g.edge(filter_for_profanity, text_chunks)

    # Example of how to use this edge without a predicated is commented out below.
    g.edge(object_detector, text_chunks, prefilter_predicates="a=one and c=three")
    #g.edge(object_detector, text_chunks)
    ...
    g.edge(text_chunks, chunk_embeddings)
